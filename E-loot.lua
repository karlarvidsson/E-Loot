...
function ELootHandleRaidUpdate(self, event, ...)
 ...
 function ELootHandleWhisper(self, event, ...)
	...
	if (event == "CHAT_MSG_WHISPER") then
		...
		if (raidIndex) then
			--DEFAULT_CHAT_FRAME:AddMessage("sender is in raid, ok");
		else
			DEFAULT_CHAT_FRAME:AddMessage("sender is not in raid, ignoring");
			return;
		end
--DEFAULT_CHAT_FRAME:AddMessage(author);
		local itemrqi = "";
...
DEFAULT_CHAT_FRAME:AddMessage(event);
 ...
 function ELootPost()
	...
	for i=1, GetNumLootItems() do
		DEFAULT_CHAT_FRAME:AddMessage("processing loot index " .. i);
		...
		if (itemlink) then
			...
			DEFAULT_CHAT_FRAME:AddMessage("loot index " .. i .. " has nil itemlink, skipping");
		end
	end
